...
import { dailyForecast, formatTemperature, isTripDay, uploadRecordFile, message, weatherCodeLabel, weatherSummary } from '../lib/api'
import { client, extension, recordFileUrl } from '../lib/trailbase'
import { useRealtime } from '../hooks/useRealtime'
import type { ActivityEvent, ChecklistItem, ItineraryItem, Profile, TemperatureUnit, Trip, TripMember, TripRole, WeatherBriefing } from '../types'
...
export function TripPage() {
  ...
  return <>
    ...
    <div className="mt-7">
      {tab === 'overview' && <Overview trip={trip.data} weather={weather.data} temperatureUnit={profile.data?.temperature_unit ?? 'C'} canEdit={canEdit} isOwner={isOwner} upload={upload} refreshWeather={refreshWeather} removeTrip={removeTrip} invalidate={invalidate} />}
...
function Overview({ trip, weather, temperatureUnit, canEdit, isOwner, upload, refreshWeather, removeTrip, invalidate }: { trip: Trip; weather?: WeatherBriefing | null; temperatureUnit: TemperatureUnit; canEdit: boolean; isOwner: boolean; upload: ReturnType<typeof useMutation<void, Error, File>>; refreshWeather: ReturnType<typeof useMutation<unknown, Error, void>>; removeTrip: ReturnType<typeof useMutation<void, Error, void>>; invalidate: () => Promise<unknown> }) {
  ...
  return <div className="grid gap-6 lg:grid-cols-[1.25fr_.75fr]">
    ...
    <div className="grid content-start gap-6"><Card className="p-6"><div className="flex items-center"><CloudSun className="mr-2 text-amber-600" /><h2 className="section-title">Weather briefing</h2></div>{weather ? <><p className="mt-4 text-lg font-semibold">{weatherSummary(weather, temperatureUnit)}</p><p className="mt-2 text-xs text-muted">Updated {new Date(weather.fetched * 1000).toLocaleString()}</p></> : <p className="mt-4 text-sm text-muted">Generate a briefing using Nominatim and Open-Meteo.</p>}{days.length ? <div className="mt-5 grid grid-cols-2 gap-2">{days.map((day) => { const tripDay = isTripDay(day.date, trip.start_date, trip.end_date); return <div key={day.date} className={`rounded-xl border p-3 ${tripDay ? 'border-amber-400 bg-amber-50 dark:bg-amber-950/30' : 'border-border bg-stone/60'}`}><div className="flex items-start justify-between gap-2"><div><p className="text-xs font-bold uppercase text-muted">{new Date(`${day.date}T00:00:00`).toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' })}</p><p className="mt-1 text-sm font-bold">{weatherCodeLabel(day.code)}</p></div><WeatherIcon code={day.code} /></div>{tripDay && <span className="mt-2 inline-block rounded-full bg-amber-200 px-2 py-0.5 text-[10px] font-black uppercase text-amber-900">Trip day</span>}<p className="mt-3 text-sm font-bold">High {formatTemperature(day.highC, temperatureUnit)} <span className="font-normal text-muted">· Low {formatTemperature(day.lowC, temperatureUnit)}</span></p><div className="mt-2 grid gap-1 text-xs text-muted"><span className="flex items-center gap-1"><Droplets size={13} />Rain: {day.rainChance ?? '—'}% chance · {day.precipitationMm ?? '—'} mm total</span><span className="flex items-center gap-1"><Wind size={13} />Wind: up to {day.windKph ?? '—'} km/h</span></div></div> })}</div> : weather && <p className="mt-4 text-sm text-muted">Refresh the briefing to load the 7-day outlook.</p>}<Button className="mt-5 w-full" variant="secondary" disabled={refreshWeather.isPending} onClick={() => refreshWeather.mutate()}><RefreshCw size={16} className={refreshWeather.isPending ? 'animate-spin' : ''} />{weather ? 'Refresh' : 'Generate'} briefing</Button>{refreshWeather.error && <p className="mt-3 text-sm text-red-700">{message(refreshWeather.error)}</p>}</Card>
