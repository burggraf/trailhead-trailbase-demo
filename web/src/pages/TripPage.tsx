...
import { ArrowLeft, CalendarDays, Check, CloudFog, CloudLightning, CloudRain, CloudSnow, CloudSun, Droplets, ImagePlus, ListChecks, LoaderCircle, Mail, MapPin, Plus, RefreshCw, Route, Sparkles, Sun, Trash2, UserPlus, Users, Wind } from 'lucide-react'
import { Link, useNavigate, useParams } from 'react-router-dom'
import { useAuth } from '../auth'
import { AuthenticatedImage } from '../components/AuthenticatedImage'
import { Badge, Button, Card, Empty, Field, Input } from '../components/ui'
import { dailyForecast, formatPrecipitation, formatTemperature, formatWind, isTripDay, uploadRecordFile, message, weatherCodeLabel, weatherSummary } from '../lib/api'
import { client, extension, recordFileUrl } from '../lib/trailbase'
import { useRealtime } from '../hooks/useRealtime'
import type { ActivityEvent, AiSuggestion, ChecklistItem, ItineraryItem, OwnerInvite, Profile, UnitSystem, Trip, TripMember, TripRole, WeatherBriefing } from '../types'
...
export function TripPage() {
  ...
  return <>
    ...
    <div className="mt-7">
      {tab === 'overview' && <Overview trip={trip.data} weather={weather.data} unitSystem={profile.data?.unit_system ?? 'metric'} canEdit={canEdit} isOwner={isOwner} upload={upload} refreshWeather={refreshWeather} removeTrip={removeTrip} invalidate={invalidate} />}
      {tab === 'itinerary' && <Itinerary trip={trip.data} userId={user!.id} items={itinerary.data ?? []} canEdit={canEdit} invalidate={invalidate} />}
...
function Overview({ trip, weather, unitSystem, canEdit, isOwner, upload, refreshWeather, removeTrip, invalidate }: { trip: Trip; weather?: WeatherBriefing | null; unitSystem: UnitSystem; canEdit: boolean; isOwner: boolean; upload: ReturnType<typeof useMutation<void, Error, File>>; refreshWeather: ReturnType<typeof useMutation<unknown, Error, void>>; removeTrip: ReturnType<typeof useMutation<void, Error, void>>; invalidate: () => Promise<unknown> }) {
  ...
  const days = weather ? dailyForecast(weather) : []
  ...
  return <div className="grid gap-6 lg:grid-cols-[1.25fr_.75fr]">
    ...
    <div className="grid content-start gap-6"><Card className="p-6"><div className="flex items-center"><CloudSun className="mr-2 text-amber-600" /><h2 className="section-title">Weather briefing</h2></div>{weather ? <><p className="mt-4 text-lg font-semibold">{weatherSummary(weather, unitSystem)}</p><p className="mt-2 text-xs text-muted">Updated {new Date(weather.fetched * 1000).toLocaleString()}</p></> : <p className="mt-4 text-sm text-muted">Generate a briefing using Nominatim and Open-Meteo.</p>}{days.length ? <div className="mt-5 grid grid-cols-2 gap-2">{days.map((day) => { const tripDay = isTripDay(day.date, trip.start_date, trip.end_date); return <div key={day.date} className={`rounded-xl border p-3 ${tripDay ? 'border-amber-400 bg-amber-50 dark:bg-amber-950/30' : 'border-border bg-stone/60'}`}><div className="flex items-start justify-between gap-2"><div><p className="text-xs font-bold uppercase text-muted">{new Date(`${day.date}T00:00:00`).toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' })}</p><p className="mt-1 text-sm font-bold">{weatherCodeLabel(day.code)}</p></div><WeatherIcon code={day.code} /></div>{tripDay && <span className="mt-2 inline-block rounded-full bg-amber-200 px-2 py-0.5 text-[10px] font-black uppercase text-amber-900">Trip day</span>}<p className="mt-3 text-sm font-bold">High {formatTemperature(day.highC, unitSystem)} <span className="font-normal text-muted">· Low {formatTemperature(day.lowC, unitSystem)}</span></p><div className="mt-2 grid gap-1 text-xs text-muted"><span className="flex items-center gap-1"><Droplets size={13} />Rain: {day.rainChance ?? '—'}% chance · {day.precipitationMm === null ? '—' : formatPrecipitation(day.precipitationMm, unitSystem)} total</span><span className="flex items-center gap-1"><Wind size={13} />Wind: up to {day.windKph === null ? '—' : formatWind(day.windKph, unitSystem)}</span></div></div> })}</div> : weather && <p className="mt-4 text-sm text-muted">Refresh the briefing to load the 7-day outlook.</p>}<Button className="mt-5 w-full" variant="secondary" disabled={refreshWeather.isPending} onClick={() => refreshWeather.mutate()}><RefreshCw size={16} className={refreshWeather.isPending ? 'animate-spin' : ''} />{weather ? 'Refresh' : 'Generate'} briefing</Button>{refreshWeather.error && <p className="mt-3 text-sm text-red-700">{message(refreshWeather.error)}</p>}</Card>
...
function WeatherIcon({ code }: { code: number }) {
  const props = { size: 22, className: 'shrink-0 text-amber-600' }
...
export function Itinerary({ trip, userId, items, canEdit, invalidate }: { trip: Trip; userId: string; items: ItineraryItem[]; canEdit: boolean; invalidate: () => Promise<unknown> }) {
  const [suggestions, setSuggestions] = useState<AiSuggestion[]>([])
  const [scheduled, setScheduled] = useState<AiSuggestion | null>(null)
  const addStarted = useRef(false)
  const suggest = useMutation({ mutationFn: () => extension<{ suggestions: AiSuggestion[] }>(`/trips/${trip.id}/suggestions`, { method: 'POST' }), onSuccess: (data) => setSuggestions(Array.from(new Map(data.suggestions.map((item) => [JSON.stringify(item), item])).values())) })
  const search = () => { if (suggestions.length && !confirm('Replace your unscheduled suggestions?')) return; suggest.mutate(undefined, { onSuccess: () => setScheduled(null) }) }
  const suggestionPanel = canEdit && <Card className="mb-6 p-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="eyebrow">Planner assistant</p><h2 className="section-title mt-1">Ideas for your trip</h2></div><Button onClick={search} disabled={suggest.isPending}><Sparkles size={16} />{suggestions.length ? 'Search again' : 'Suggest things to do'}</Button></div>{suggest.isPending && <p role="status" className="mt-4 flex items-center gap-2 text-sm text-muted"><LoaderCircle size={16} className="animate-spin" />Searching for events and local attractions…</p>}{suggest.error && <div role="alert" className="mt-4 flex items-center justify-between gap-3 text-sm text-red-700"><span>{message(suggest.error)}</span><Button variant="secondary" onClick={search}>Try again</Button></div>}{suggestions.length > 0 && <div className="mt-5 grid gap-3">{suggestions.map((item) => <Card key={JSON.stringify(item)} className="p-5"><div className="flex items-start justify-between gap-3"><div><Badge tone="amber">{item.type}</Badge><h3 className="mt-2 font-bold">{item.title}</h3></div><Button variant="ghost" aria-label={`Dismiss ${item.title}`} onClick={() => { setSuggestions((all) => all.filter((s) => s !== item)); if (scheduled === item) setScheduled(null) }}>Dismiss</Button></div><p className="mt-2 text-sm text-muted">{item.description}</p><p className="mt-2 text-sm">{item.place} · {item.date} · {item.time}</p>{Array.from(new Map(item.sources.filter((source) => source.url.startsWith('https://')).map((source) => [`${source.url}\u0000${source.title}`, source])).values()).map((source) => <a className="link mt-2 block text-sm" key={`${source.url}\u0000${source.title}`} href={source.url} target="_blank" rel="noreferrer">{source.title}</a>)}<Button className="mt-4" variant="secondary" onClick={() => setScheduled(item)}>Schedule</Button></Card>)}</div>}{suggestions.length > 0 && <Button className="mt-4" variant="ghost" onClick={() => { setSuggestions([]); setScheduled(null) }}>Clear suggestions</Button>}</Card>
  const add = useMutation({ mutationFn: async (record: Partial<ItineraryItem>) => {
    const id = await client.records('itinerary_items').create(record)
    try {
      await client.records('activity_events').create({ trip_id: trip.id, actor: userId, kind: 'itinerary_added', summary: `Added ${record.title} to the itinerary` })
    } catch (error) {
      await client.records('itinerary_items').delete(id)
      throw error
    }
    return id
  }, onSuccess: invalidate, onSettled: () => { addStarted.current = false } })
  const remove = useMutation({ mutationFn: (id: string) => client.records('itinerary_items').delete(id), onSuccess: invalidate })
  const submit = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); if (add.isPending || addStarted.current) return; addStarted.current = true; const form = event.currentTarget; const data = new FormData(form); add.mutate({ trip_id: trip.id, created_by: userId, day: String(data.get('day')), start_time: data.get('start_time') ? `${String(data.get('start_time'))}:00` : '', title: String(data.get('title')), place: String(data.get('place')), notes: '', position: items.length }, { onSuccess: () => form.reset() }) }
  const schedule = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); if (add.isPending || addStarted.current) return; addStarted.current = true; const form = event.currentTarget; const data = new FormData(form); add.mutate({ trip_id: trip.id, created_by: userId, day: String(data.get('day')), start_time: data.get('start_time') ? `${String(data.get('start_time'))}:00` : '', title: String(data.get('title')), place: String(data.get('place')), notes: '', position: items.length }, { onSuccess: () => { setSuggestions((all) => all.filter((s) => s !== scheduled)); setScheduled(null) } }) }
  return <div>{suggestionPanel}{scheduled && <Card className="mb-6 p-6"><h3 className="font-bold">Schedule {scheduled.title}</h3><form className="mt-4 grid gap-4" onSubmit={schedule}><Field label="Title"><Input name="title" defaultValue={scheduled.title} required /></Field><Field label="Place"><Input name="place" defaultValue={scheduled.place} /></Field><Field label="Date"><Input type="date" name="day" defaultValue={scheduled.date} min={trip.start_date} max={trip.end_date} required /></Field><Field label="Time"><Input type="time" name="start_time" defaultValue={scheduled.time} /></Field><Button disabled={add.isPending}>Add to itinerary</Button>{add.error && <p role="alert" className="text-sm text-red-700">{message(add.error)}</p>}</form></Card>}<div className="grid gap-6 lg:grid-cols-[.7fr_1.3fr]">{canEdit && <Card className="h-fit p-6"><div className="flex items-center gap-2"><Plus className="text-amber-600" /><h2 className="section-title">Add a stop</h2></div><form className="mt-5 grid gap-4" onSubmit={submit}><Field label="Date"><Input type="date" name="day" min={trip.start_date} max={trip.end_date} required /></Field><Field label="Time"><Input type="time" name="start_time" /></Field><Field label="What’s happening?"><Input name="title" placeholder="Sunrise hike" required /></Field><Field label="Place"><Input name="place" placeholder="Tre Cime trailhead" /></Field><Button disabled={add.isPending}>Add to itinerary</Button>{add.error && <p role="alert" className="text-sm text-red-700">{message(add.error)}</p>}</form></Card>}<section><h2 className="section-title">Itinerary</h2>{items.length ? <div className="mt-5 grid gap-3">{items.map((item) => <Card key={item.id} className="flex items-start gap-4 p-5"><div className="min-w-20 rounded-xl bg-stone p-3 text-center"><p className="text-xs font-bold uppercase text-muted">{new Date(`${item.day}T00:00:00`).toLocaleDateString(undefined,{month:'short',day:'numeric'})}</p><p className="mt-1 text-sm font-semibold">{item.start_time || 'Anytime'}</p></div><div className="mr-auto"><h3 className="font-bold">{item.title}</h3>{item.place && <p className="mt-1 flex items-center gap-1 text-sm text-muted"><MapPin size={14} />{item.place}</p>}</div>{canEdit && <Button variant="ghost" className="size-9 px-0" aria-label={`Delete ${item.title}`} onClick={() => remove.mutate(item.id)}><Trash2 size={16} /></Button>}</Card>)}</div> : <div className="mt-5"><Empty title="Nothing scheduled">Add the first stop to shape the trip together.</Empty></div>}</section></div></div>
}
...
function Checklist({ tripId, userId, items, canEdit, invalidate }: { tripId: string; userId: string; items: ChecklistItem[]; canEdit: boolean; invalidate: () => Promise<unknown> }) {
  const add = useMutation({ mutationFn: async (text: string) => {
    const id = await client.records('checklist_items').create({ trip_id: tripId, created_by: userId, text, completed: 0, position: items.length })
    await client.records('activity_events').create({ trip_id: tripId, actor: userId, kind: 'checklist_added', summary: `Added “${text}” to the checklist` })
    return id
  }, onSuccess: invalidate })
  const toggle = useMutation({ mutationFn: async (item: ChecklistItem) => {
    await client.records('checklist_items').update(item.id, { completed: item.completed ? 0 : 1 })
    if (!item.completed) await client.records('activity_events').create({ trip_id: tripId, actor: userId, kind: 'checklist_completed', summary: `Completed “${item.text}”` })
  }, onSuccess: invalidate })
...
export function Members({ tripId, members, isOwner, invalidate }: { tripId: string; members: TripMember[]; isOwner: boolean; invalidate: () => Promise<unknown> }) {
  const queryClient = useQueryClient()
  const pending = useQuery({ queryKey: ['trip', tripId, 'invites'], queryFn: async () => (await extension<{ records: OwnerInvite[] }>(`/trips/${tripId}/invites`)).records, enabled: isOwner })
  const refreshInvites = () => queryClient.invalidateQueries({ queryKey: ['trip', tripId, 'invites'] })
  const invite = useMutation({ mutationFn: (input: { email: string; role: string }) => extension<{ id: string; delivery: string }>(`/trips/${tripId}/invites`, { method: 'POST', body: JSON.stringify(input) }), onSuccess: refreshInvites })
  const resend = useMutation({ mutationFn: (id: string) => extension(`/trips/${tripId}/invites/${id}/resend`, { method: 'POST' }), onSuccess: refreshInvites })
  const cancel = useMutation({ mutationFn: (id: string) => extension(`/trips/${tripId}/invites/${id}`, { method: 'DELETE' }), onSuccess: refreshInvites })
  ...
  const error = invite.error || resend.error || cancel.error

  return <div className="grid gap-6 lg:grid-cols-[1.3fr_.7fr]"><section><h2 className="section-title">Travelers</h2><div className="mt-5 grid gap-3">{members.map((member) => <Card key={member.id} className="flex items-center gap-4 p-4">{member.avatar_url ? <img className="size-11 rounded-xl object-cover" src={new URL(member.avatar_url, client.base).toString()} alt="" /> : <span className="grid size-11 place-items-center rounded-xl bg-stone font-bold">{member.display_name[0]}</span>}<div className="mr-auto"><h3 className="font-bold">{member.display_name}</h3><p className="text-sm text-muted">Joined {new Date(member.joined * 1000).toLocaleDateString()}</p></div>{isOwner && member.role !== 'owner' ? <><select className="input w-auto" value={member.role} aria-label={`Role for ${member.display_name}`} onChange={(event) => update.mutate({ id: member.id, role: event.target.value as TripRole })}><option value="editor">editor</option><option value="viewer">viewer</option></select><Button variant="ghost" className="size-9 px-0" onClick={() => remove.mutate(member.id)} aria-label={`Remove ${member.display_name}`}><Trash2 size={16} /></Button></> : <Badge tone={member.role === 'owner' ? 'amber' : 'neutral'}>{member.role}</Badge>}</Card>)}</div>
    {isOwner && !!pending.data?.length && <><h3 className="mt-8 font-bold">Pending invitations</h3><div className="mt-3 grid gap-3">{pending.data.map((item) => <Card key={item.id} className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center"><span className="grid size-11 shrink-0 place-items-center rounded-xl bg-stone text-forest"><Mail size={18} /></span><div className="mr-auto"><p className="font-bold">{item.email}</p><p className="text-sm text-muted">Invited as {item.role} · expires {new Date(item.expires * 1000).toLocaleDateString()}</p><p className={`mt-1 text-xs font-semibold ${item.email_status === 'failed' ? 'text-red-700' : 'text-muted'}`}>{item.email_status === 'failed' ? 'Delivery failed' : item.email_status === 'sent' ? 'Email sent' : 'Sending email…'}</p></div><select className="input w-auto" value={item.role} aria-label={`Invited role for ${item.email}`} onChange={(event) => invite.mutate({ email: item.email, role: event.target.value })}><option value="editor">editor</option><option value="viewer">viewer</option></select><Button variant="ghost" className="size-9 px-0" aria-label="Resend invitation" disabled={resend.isPending} onClick={() => resend.mutate(item.id)}><RefreshCw size={16} /></Button><Button variant="ghost" className="size-9 px-0" aria-label="Cancel invitation" disabled={cancel.isPending} onClick={() => cancel.mutate(item.id)}><Trash2 size={16} /></Button></Card>)}</div></>}
  </section>{isOwner && <Card className="h-fit p-6"><div className="flex items-center gap-2"><UserPlus className="text-amber-600" /><h2 className="section-title">Invite someone</h2></div><form className="mt-5 grid gap-4" onSubmit={(event) => { event.preventDefault(); const form = event.currentTarget; const data = new FormData(form); invite.mutate({ email: String(data.get('email')), role: String(data.get('role')) }, { onSuccess: () => form.reset() }) }}><Field label="Email"><Input type="email" name="email" required /></Field><Field label="Role"><select className="input" name="role"><option value="editor">Editor — can plan</option><option value="viewer">Viewer — read only</option></select></Field><Button disabled={invite.isPending}>{invite.isPending ? 'Sending…' : 'Send invitation'}</Button></form>{invite.data && <p role="status" className={`mt-3 text-sm ${invite.data.delivery === 'sent' ? 'text-emerald-700' : 'text-red-700'}`}>{invite.data.delivery === 'sent' ? 'Invitation email sent.' : 'Invitation saved, but email delivery failed. You can resend it below.'}</p>}{error && <p className="mt-3 text-sm text-red-700">{message(error)}</p>}</Card>}</div>
}
