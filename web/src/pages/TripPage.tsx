...
import { ArrowLeft, CalendarDays, Check, CloudFog, CloudLightning, CloudRain, CloudSnow, CloudSun, Droplets, ImagePlus, ListChecks, Mail, MapPin, Plus, RefreshCw, Route, Sun, Trash2, UserPlus, Users, Wind } from 'lucide-react'
...
import type { ActivityEvent, ChecklistItem, ItineraryItem, OwnerInvite, Profile, UnitSystem, Trip, TripMember, TripRole, WeatherBriefing } from '../types'
...
function Itinerary({ tripId, userId, items, canEdit, invalidate }: { tripId: string; userId: string; items: ItineraryItem[]; canEdit: boolean; invalidate: () => Promise<unknown> }) {
  const add = useMutation({ mutationFn: async (record: Partial<ItineraryItem>) => {
    ...
    await client.records('activity_events').create({ trip_id: tripId, actor: userId, kind: 'itinerary_added', summary: `Added ${record.title} to the itinerary` })
    return id
